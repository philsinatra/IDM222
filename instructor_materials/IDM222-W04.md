...
^ All these columns should be floating to the left, and have a padding of 16px:
...
^ To support a 4k display, you'll need a large, high res image. Using this technique on a mobile device, the device downloads this very large file and then has to scale the image down to fit within your mobile layout. That takes a fair amount of processing power and device memory.

^ The file size of this image will also be large, larger than is needed in the mobile environment. The mobile device is never going to render the image at 1200 pixels, why should a mobile user have to download that large file?
...
^ 1. Are images need to be able to render crisply at different _device-pixel-ratios_. We want high resolution screens to get high resolution images, But we don't want to send those images to users who wouldn't see all of those extra pickles. 2. If our layout is fluid, then our images will need to squish and stretch to fit it. 3. Cases one and two are very closely related. Will tackle both problems simultaneously using the _variable-size-image_ use case. 4. Sometimes will need to adapt our images, For example crop the image or even slightly alter the content. 5. Finally, different browser support different image formats.[^1]
...
^ Let's start simply, with a fixed with image that we want to adapt to varying device pixel refuse. To do this, will use the first tool that the new spec gives us for grouping and describing image sources: the `srcset` attribute. Say we have two versions of an image, we want to send large.jpg only do users with high resolution screens. Using source that, we markup or image like this:[^1]
...
^ What if, instead of all the squishing/stretching/adapting, we simply want to give a new file format a spin, and provide a fallback for non-supporting browsers. If the browser doesn't understand the `image/svg` media type, it skips the first `source`; if it can't figure out `image/png`, it falls back to `img` and the GIF file.[^1]
