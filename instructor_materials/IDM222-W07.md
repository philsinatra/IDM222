...
^ If the `doctype` tells the browser what _kind_ of document it is serving, then the `<html>` element's `lang` attribute tells it which language it is written in.
...
^ Since `<main>` is designed to contain the unique content of a page, it can make writing a print style sheet easier. There are browser extensions available that eliminate subsidiary content in an effort to improve on screen reading experience.
...
![fit](http://digm.drexel.edu/crs/IDM222/presentations/images/contrast-gray.png)
